...
import { loadTestData, loadSchemaBinary, loadPersonDataSproto, loadAddressBookSproto, sproto } from './helpers.js';
...
describe('schema binary format - additional edge cases', () => {
  ...
  describe('countArray edge cases', () => {
    it('should return -1 when remainingLength < SIZEOF_LENGTH', () => {
      ...
      const schema = new Uint8Array([0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
      const sp = sproto.createNew(schema);
      expect(sp).toBeNull();
    });
